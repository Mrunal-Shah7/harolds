...
export default function CheckoutLayout({ children }: Readonly<{ children: ReactNode }>) {
  const { collectJsUrl, tokenizationKey } = getNmiBrowserConfig();
  return <NmiCheckoutConfigProvider config={{ collectJsUrl, tokenizationKey }}>{children}</NmiCheckoutConfigProvider>;
}
