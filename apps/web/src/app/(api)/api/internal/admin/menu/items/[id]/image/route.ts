// SPRINT-12: POST /api/internal/admin/menu/items/[id]/image — authenticated upload.
import { updateItem } from "@harolds/db";
...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const row = await updateItem(id, { imageUrl: stored.imageUrl }, session.userId);
    return adminOk({ item: row, urls: stored.urls, hash: stored.hash });
  } catch (err) {
...
export async function DELETE(request: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const row = await updateItem(id, { imageUrl: null }, session.userId);
    return adminOk({ item: row });
  } catch (err) {
    return adminAuthError(err);
  }
}
